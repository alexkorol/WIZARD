"""Package Blender's actual 48x96 pixels, never resize a generated image."""
    ...
    for gait in ['walk','sprint']:
        for direction in ['front','right','back','left']:
            ...
            for i in range(1,9):
                ...
                im=Image.open(src).convert('RGBA');assert im.size==(48,96)
                ...
                bbox=rgba.getbbox()
                assert bbox[0]>0 and bbox[1]>0 and bbox[2]<48 and bbox[3]<96,f'Clipped render: {src.name} {bbox}'
                frames.append({'src':'blender/frames/'+dest.name,'width':48,'height':96,'anchor':framing[direction]['anchor']})
            ...
            clips[key]={'frames':frames,'cloth_simulation':simulation.exists(),'cloth_loop_max_delta_px':seam,'note':
...
for sex in ['male','female']:
    for gait in ['walk','sprint']:
        ...
        for i in range(8):
            ...
            sheet.paste(im,((i%4)*48,(i//4)*96))
        sheet.save(R/f'{sex}-{gait}-right-native.png')
        sheet.resize((1536,1536),Image.Resampling.NEAREST).save(R/f'{sex}-{gait}-right-guide-8x.png')

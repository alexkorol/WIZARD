...
manifest=json.loads((R/'sheet-manifest.json').read_text());report={'scope':'Village-defense prologue ONLY','status':'First cast art study; not production animation or approved canon','alpha_origin':'Locally derived; all three raw RGB generations failed true-alpha request','layouts':{},'actors':{}}
for name,size in {'villagers':6,'defense':6,'beasts':5}.items():
 ...
 for e,box,cell,b in cells:
  ...
  entries.append({**e,'final_bbox':f.getbbox(),'source_crop':box,'foreground_pixels_preserved':True,'final_sha256':hashlib.sha256(p.read_bytes()).hexdigest()})
  report['actors'][e['actor']]={'frame_size':[w,h],'directions':['front','right','back','left'],'source_blend':f"sources/{e['actor']}.blend",'kind':'monster candidate' if name=='beasts' else 'villager candidate'}
 ...
 report['layouts'][name]={'recovered_dimensions':im.size,'frame_size':[w,h],'shared_integer_translation':[dx,dy],'chosen_detector_candidate':chosen,'automatic_result_retained':f'reconstructed/{name}-auto.png','entries':entries}
 print(name,im.size,'shift',dx,dy,'bboxes',[e['final_bbox'] for e in entries])
old=R.parent/'player-unarmed-v14-sandals-alpha'
for sex in ('male','female'):
 actor='player-'+sex
 for d in ('front','right','back','left'):shutil.copy2(old/'frames'/f'{sex}-{d}-48x96.png',R/'frames'/f'{actor}-{d}.png')
 report['actors'][actor]={'frame_size':[48,96],'directions':['front','right','back','left'],'source_study':'../player-unarmed-v14-sandals-alpha','kind':'existing unarmed player; unchanged pixels'}
shutil.copy2(old/'final/2x4-local-alpha-sheet.png',R/'final/players-sheet.png')

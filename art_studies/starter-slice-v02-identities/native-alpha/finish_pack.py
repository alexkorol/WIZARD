...
manifest=json.loads((R/'sheet-manifest.json').read_text());report={'scope':'Village-defense prologue ONLY','status':'Revised individual human idle studies; not production animation or approved canon','alpha_origin':'Native RGBA returned by all four final imagegen edits; returned alpha thresholded128 for binary sprite pixels. Earlier clean edit inputs used native returned alpha.','layouts':{},'actors':{}}
for name,size in {'players':6,'villagers':6,'defense':6,'beasts':5}.items():
 ...
 for e,box,cell,b in cells:
  ...
  report['actors'][e['actor']]={'frame_size':[w,h],'directions':['front','right','back','left'],'source_blend':(f"../../starter-slice-v01/sources/{e['actor']}.blend" if name=='beasts' else f"../sources/{e['actor']}.blend"),'kind':'monster candidate' if name=='beasts' else 'starter player candidate' if name=='players' else 'villager candidate'}
...
for scale in (1,2):
 ...
 draw.text((8,ch*2+5),f'Prologue cast candidates | {scale}x actual pixels | native returned alpha',font=font,fill='#26312c');board.save(R/'review'/f'prologue-cast-{scale}x.png')

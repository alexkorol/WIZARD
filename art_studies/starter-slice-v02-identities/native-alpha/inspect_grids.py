...
for name,sizes in {'players':[5,6,7],'villagers':[5,6,7],'defense':[5,6,7],'beasts':[4,5,6]}.items():
 rec=PixelArtReconstructor(str(R/'inputs'/f'{name}.png'))
 ...
 previews=[]
 for size in sizes:
  c=next((c for c in fit['debug']['candidates'] if c['size']==size),None)
  if c is None:continue
  rec.cell_size=size;rec.offset=tuple(c['offset'])

...
with zipfile.ZipFile(R/'prologue-cast-frames-v02.zip','w',zipfile.ZIP_DEFLATED) as z:
 for folder in ['frames','final']:
  for p in (R/folder).glob('*.png'):z.write(p,p.relative_to(R))
 for f in ['README.md','pack-verification.json','asset-hashes.json','owner-reference-directions.json','generation-history.json']:z.write(R/f,f)
 for p in (R/'review').glob('prologue-cast-*.png'):z.write(p,p.relative_to(R))
with zipfile.ZipFile(R/'prologue-cast-frames-v02.zip') as z:

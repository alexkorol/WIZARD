...
## 2026-07-13 — Post-overhaul: ascendancy circles replace the old annexes

- The six 5-6 node subtrees (Genius Circle, Ranger's Writ, Vanguard Oath, Spellblade Annex, Skirmish Annex, Seer's Annex) are deleted. In their place, each rim unlock corner opens a 13-passive ascendancy circle themed to its armoury window: Attendant (INT — ward, spirit, rite speed, cooldowns), Quick Rig (DEX — attack speed, evasion, tool play), Warcall (STR — life, guard, horn calls), Preparations (DEX/INT — vials, traps, ailments), Trophies (DEX/STR — crit, spoils, beast damage), and Relics (INT/STR — relics, resistances, companions).
- Naming rule, per Alexei: no invented lore titles for the circles. A first pass shipped "Attendant's Vigil" / "Hunter's Wall" style labels and got called out as AI shorthand — each circle is simply named after the corner that opens it.
- Composition per circle: one entry notable, eight smalls, two more notables, one mastery, one capstone keystone. All stat ids resolve against the shared registry; keystones are rule-text with a real cost, per the house style.
- Shape, per Alexei: the first pass used irregular two-lane layouts; all six circles now share one seed-of-life wheel — a hub seat (the mastery), the six circle-center seats at radius 96, and the six outer-intersection seats at radius 96·√3 ≈ 166, with the entry notable on the intersection facing the gate and the keystone at the apex. Thirty conduits per wheel, every one a lattice edge (96 between touching circles, 166 along the rim); a smoke test asserts all 180 wheel edges sit on that grid.
- Subtree ids renamed to match their corners (`attendant`, `quickrig`, `warcall`, `preparations`, `trophies`, `relics`); `GATE_UNLOCKS` keys follow. The unlock rule was then simplified on Alexei's call: allocating the corner seat alone opens its circle — the old "complete any inner six-node circle" shape-combo requirement is gone. One click on the corner now grants the armoury window and reveals the ascendancy.
- The renderer now sizes each subtree halo and label from the authored layout extents instead of a fixed 150px radius, so the deeper circles stay enclosed (measured ~204px).
- Runtime smoke updated (new root ids) plus a new test asserting every circle holds 10-15 passives with exactly one keystone and one mastery, that allocating a corner reveals only its own circle, and that refunding the corner closes it again. Full suite green.
- Browser-verified over localhost: closed a real circle next to the Attendant corner, watched the Attendant circle appear with all 13 seats while the other five stayed hidden, allocated two seats through the normal click path (4 points, Ward 180 → 268), and confirmed note pins on circle seats hide when the circle closes. Console clean. All content lives in index.html's SUBTREES, so the v=13 asset pins stand.

## 2026-07-13 — Post-overhaul: player notes + rim unlock corners

- Player-facing annotation UI: a Notes toolbar toggle (hotkey `N`) turns clicks into note edits on any node or conduit; Alt+click annotates without the mode; gold pins mark noted targets — hover a pin for the text, click it to edit; the selection panel shows the selected node's note and carries an Add/Edit Note button. Notes are HTML-escaped, capped at 500 chars, ride the build code (optional `a` field, still v1 — old codes import unchanged, and a code without notes leaves existing notes alone), and deliberately survive reset and undo: they are plan marks, not allocations.
- The six ring-10 rim corners are now armoury unlock seats per the outer-hex plan, alternating slot/pack around the rim: Attendant (INT, extra focus slot), Relics (INT/STR, Reliquary pack), Warcall (STR, War-call slot), Trophies (DEX/STR, Spoils Roll pack), Quick Rig (DEX, Quick Rig slot), Preparations (DEX/INT, Preparation Case pack). They keep their gateway role for the outer circles; allocating one publishes its flag over the existing bridge (`GATE_UNLOCKS` reuses the CLASS_UNLOCKS flag vocabulary, so Vesselforge needed zero changes). Class milestones keep their own unlocks — the corners are the class-agnostic route to the same windows.
- The Calling panel now lists armoury unlocks even before any calling exists, since corners grant them class-free.
- Two new runtime smoke tests: rim corners grant and revoke their flags over the bridge; notes store, pin, round-trip through build codes, survive reset, and save through the editor controller. Full suite green (runtime smoke 15, patterns 12, jewels 3, tree-data 6, progression 3, balance 3, verdigris-stats 44, pack 41).
- Browser-verified over localhost: wrote a node note and a conduit note through the real editor, two pins rendered and survived a reload, and activating the Attendant corner surfaced the "Open Attendant" tab on the Vesselforge page with no class allocated. No asset changes, so the v=13 pins stand. (Preview-pane screenshots time out on this page — the always-on ring-spin animations stall frame capture — so verification used DOM checks.)
...
- Class rule: the FIRST class milestone allocated marks the character's Calling; every active class milestone contributes its armoury unlocks. Refunding the Calling promotes the next-allocated class. Covered by a runtime smoke test.
- Unlock flags ride the stat sheet (`stats.characterClass`, `stats.unlocks`) per the `CLASS_UNLOCKS` table. The six inventory windows map to Champion → `war_call_slot`, Acrobat → `quick_rig_slot`, Archmage → `attendant_focus_slot`, Reaver → `spoils_pack`, Nightblade → `preparations_pack`, and Ritualist → `reliquary_pack`, alongside their existing combat flags.
...
- Vesselforge consumes six independent flags and renders one `<<` tab per unlocked 2x2 auxiliary seat or 4x4 specialty pack. Unavailable windows remain absent; the page re-reads the bridge on storage events, window focus, and a slow poll.

...
- The imported world projects the feather-stitched 4K varied-territory atlas onto the complete upward-facing mesh. A second feather-stitched 4K RGB illumination map adds true emissive lava fissures, low cyan reef life, and rare pulsing grove or settlement points; each channel is terrain-gated so cyan reef light stays in confirmed shallow water while lava and restrained grove light remain on land. A stable height/atlas-masked water skin follows its sea exactly: deep basins remain navy, turquoise is delayed until the actual shelf band, and blue/cyan atlas noise on elevated rock is neutralized. Broad geometric swell is disabled; only shoreline topography drives animated surf normals and breaking foam, while deep water keeps restrained reflection detail, warm directional glints, and fine sparkles. At grazing camera angles, the sun's live world position and camera position define a widening specular road across the ocean. A compact center-weighted far-depth blur softly defocuses only the receding rim without separated bokeh copies, and AgX tone mapping plus calmer key/rim lighting preserve midtone texture instead of crushing shadows and clipping highlights. Seven ray-marched ellipsoidal weather volumes create internally layered thunderheads, rain shelves, sunlit clouds, high cloud decks, and low local fog banks with true camera parallax. Nine independently turbulent aurora shells form three depth-stacked arcs rather than repeated flat curtains. The world retains seven perimeter waterfalls, now rendered as broken translucent water ribbons with low horizontal base mist rather than additive light pillars, plus lightning, sun shafts, and a cyan/violet nebula field. Broken neutral glacial shelves remain around the imported rim, with clustered icicles and actual geometry gaps wherever a waterfall crosses; shelf breakup leaves natural stone between icy regions instead of drawing a uniform blue ring. Below the rim, the real Meshy relief is retained and textured, then reshaped in Blender: the outer hanging mass is compressed and the deepest authored forms pull inward to one tapered spinning-top peak. The former loose black cone field remains suppressed when the runtime mesh loads.
...
Geometry is reused across tiers; controls change draw ranges, instance counts, shadows, pixel ratio, and far-depth blur radius without rebuilding the scene. The renderer uses AgX tone mapping, explicit DPR caps, batched props, and a center-weighted 13-tap far-only depth-of-field compositor with no bloom pass.

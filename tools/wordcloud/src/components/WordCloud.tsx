...
interface NodeWithPosition extends WordCloudWord {
  x: number;
  y: number;
  vx: number;
  vy: number;
...
const useForceLayout = (containerRef: React.RefObject<HTMLDivElement>, words: WordCloudWord[]) => {
  ...
  const nodesRef = useRef<NodeWithPosition[]>(words.map(word => ({
    ...word,
    x: Math.random() * 1000,
    y: Math.random() * 600,
    vx: 0,
  ...
  useEffect(() => {
    ...
    const simulate = () => {
      ...
      setPositions(prev => {
        const next = nodes.reduce<Positions>((acc, node) => ({
          ...acc,
          [node.id]: { x: node.x, y: node.y }
        }), {});
        
        return Object.keys(next).reduce<Positions>((acc, key) => ({
          ...acc,
          [Number(key)]: prev[Number(key)] ? {
            x: prev[Number(key)].x * 0.1 + next[Number(key)].x * 0.9,
            y: prev[Number(key)].y * 0.1 + next[Number(key)].y * 0.9
          } : next[Number(key)]
        }), {});
      });
